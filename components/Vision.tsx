import Image from "next/image";
import React from "react";
import Vision from "../public/Vision.svg";

const AboutUsSection = () => {
  return (
    <main className="overflow-x-hidden md:mt-10 ">
      {" "}
      <section className="main-container mb-0 xl:flex xxl:items-start justify-start text-white"id="vision">
        {/* hero text container */}
        <div className="flex flex-col p-5 justify-center lg:pt-20">
          <div className="flex mb-10">
            <h1 className="m-0 relative text-inherit leading-[72px] font-bold font-poppins inline-block [filter:drop-shadow(0px_4px_14px_rgba(0,_0,_0,_0.5))] max-w-full z-[3] sm:text-17xl sm:leading-[43px] lg:text-[48px] lg:leading-[58px]">
              <span className="text-transparent !bg-clip-text [background:linear-gradient(180deg,_#fff,_rgba(255,_255,_255,_0))] [-webkit-background-clip:text] [-webkit-text-fill-color:transparent]">
                OUR
              </span>
              <span className="text-">{` `}</span>
              <span className=" text-indigo-500">VISION</span>
            </h1>
          </div>

          <div className=" max-w-lg relative text-xl leading-[29px] font-inter text-lightsteelblue z-[1] mq450:text-base mq450:leading-[23px]">
            <p className="m-0 leading-9 ">
              Our vision is to illuminate Dorchester County with a beacon of
              educational and spiritual excellence embodied by Living Word
              Christian Academy. By fostering spirituality, nurturing curiosity,
              and promoting community engagement, we aim to forge leaders who
              inspire positive change.
            </p>
          </div>
        </div>
        <div className="flex md:ml-[200px]">
          <div className="w-full xl:flex relative z-10 lg:pl-12">
            <Image
              src={Vision}
              alt="Vision"
              quality={90}
              className="scale-95 xl:scale-100"
            />
          </div>
        </div>
      </section>
    </main>
  );
};

export default AboutUsSection;
